...
double CalcMatMulTime_kij_opt_my(double * A, double * B, double * C, size_t N)
{
    struct timeval start, end;
    double r_time = 0.0;
    size_t i, j, k;
    ...
    for (k = 0; k < N; k++)
        for(i = 0; i < N; i++)
        {
            ...
            b_ind = k * N;
            for(j = 0; j < N; j++)
                C[c_ind + j] = C[c_ind + j] + A[a_ind] * B[b_ind + j];
        }
    gettimeofday(&end, NULL);
...
// kij ordering naive optimization (a_ind, b_ind)
    average_runtime = 0.0;
    for(int n=0; n<NRuns; n++)
    {
        runtimes[n]=CalcMatMulTime_kij_opt_my(&A[0], &B[0], &C[0], N);
        printf("runtime %lf seconds\n", runtimes[n]);
        average_runtime += runtimes[n]/NRuns;
    }
